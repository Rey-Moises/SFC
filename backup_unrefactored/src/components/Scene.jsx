 ...
 *
 *  Phase 1 ANTICIPATION (0.18s) — scale ticks to 0.25 + slight wind-up rotation.
 *                                 Tiny pre-coil, like a spring loading.
 *  Phase 2 BURST         (0.55s) — explosive elastic blast to target position + scale.
 *                                 Fast rotation, big overshoot. The hero moment.
 *  Phase 3 SETTLE        (0.45s) — micro-bounce, breathing, locks into place.
 *  Phase 4 DRIFT         (cont.) — slow gravity tumble. Scroll cue.
 */
function ExplodingObject({ targetPosition, targetScale, delay, fallSpeed = 0.15, children }) {
  ...
  useFrame((state, delta) => {
    ...
    // PHASE 1 — ANTICIPATION: tiny scale-up + slow wind-up rotation
    if (phase.current === 'anticipation') {
      ...
      // Slight wind-up rotation (back-cock before the throw)
      ref.current.rotation.z = -windDir.current * 0.3 * t
      // Hold at origin during anticipation
    ...
    // PHASE 3 — SETTLE: micro-bounce, gentle breathing
    ...
    // PHASE 4 — DRIFT: slow gravity + tumble (scroll cue)
...
export default function Scene({ mouse, scrollY = 0 }) {
  ...
  const beanData = useMemo(() => {
    const heroBeansArr = Array.from({ length: 4 }, () => ({
      ...
      scale: 0.62 + Math.random() * 0.28, // Larger hero beans
      fallSpeed: 0.05 + Math.random() * 0.06,
      delay: 0.08 + Math.random() * 0.25,
    }))

    const midBeansArr = Array.from({ length: 7 }, () => ({
      position: [
        (Math.random() - 0.5) * 20,
    ...
    const farBeansArr = Array.from({ length: 5 }, () => ({
      position: [
        (Math.random() - 0.5) * 22,
    ...
    return [...heroBeansArr, ...midBeansArr, ...farBeansArr]
  }, [])

  // Fudge chunks — tiered, slightly fewer so beans dominate
  ...
  useFrame((state, delta) => {
    ...
    // Mouse parallax — slightly more pronounced
    ...
    //   Starts pulled back at z=8.5, then pushes in to z=5 during the burst window
    //   (matches the bean explosion timing).
    const { camera } = state
    const t = sceneClock.current
    // Dolly curve: pull-back held until t=0.2, then ease in to final at t=1.4
  ...
  return (
    <>
      {/* CINEMATIC LIGHTING — strong key + warm rim, no fill flooding */}
      ...
      <directionalLight
        ...
        color="#FFD8B8"
      />
      {/* Subtle accent rim from above for definition */}
      <directionalLight
        position={[0, 6, 2]}
        intensity={0.45}
        color="#FFFFFF"
      ...
      {/* CRISP cinematic post — no DOF, stronger bloom, soft vignette */}
